...
func Connect(c *gin.Context) {
	...
	if err != nil {
		log.Errorf("failed to create h265 websocket: %s", err)
		return
	}
	defer func() {
		_ = wsConn.Close()
		log.Debugf("h265 websocket disconnected: %s", c.ClientIP())
	}()
	log.Debugf("h265 websocket connected: %s", c.ClientIP())
	...
	if err != nil {
		log.Errorf("failed to create h265 media engine: %s", err)
		return
	}

	videoConn, err := createPeerConnection(iceServers, mediaEngine)
	if err != nil {
		log.Errorf("failed to create h265 video peer connection: %s", err)
		return
	}
	defer func() {
		_ = videoConn.Close()
		log.Debugf("h265 video peer disconnected: %s", c.ClientIP())
	}()

	// create audio connection
	audioConn, err := createPeerConnection(iceServers, nil)
	if err != nil {
		log.Errorf("failed to create h265 audio peer connection: %s", err)
		return
	}
	defer func() {
		_ = audioConn.Close()
		log.Debugf("h265 audio peer disconnected: %s", c.ClientIP())
	}()

	// create client
	client := NewClient(wsConn, videoConn, audioConn)

	track, err := client.AddTrack()
	if err != nil {
	...
	manager := getManager()
	manager.AddTrack(wsConn, track)
	defer manager.RemoveTrack(wsConn)

	// handle signaling
	signalingHandler := NewSignalingHandler(client, track)
...
func createMediaEngine() (*webrtc.MediaEngine, error) {
	...
	if err := mediaEngine.RegisterCodec(codec, webrtc.RTPCodecTypeVideo); err != nil {
		log.Errorf("failed to register codec: %s", err)
		return nil, err
	}
